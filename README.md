...
编写 FTLSoul Skill 时使用 `ftl-skill-authoring`。它从当前源码位置或 `scripts/link-skills.ps1` 维护的安装清单确定真实克隆目录，验证 Git 根目录和仓库签名后才允许编辑；用户安装目录和插件缓存只作为发现表面，不作为源码副本。普通 Markdown 段落和列表项保持一个语义块一个物理行，不按固定显示宽度硬折行；仓库内置校验脚本会在提交前检查这一约束。
